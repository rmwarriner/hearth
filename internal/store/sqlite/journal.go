...
func (s *Store) CreateJournalEntry(ctx context.Context, e journal.JournalEntry) error {
	...
	_, err = tx.ExecContext(ctx,
		...
		e.PostedAt.UTC().Format(time.RFC3339Nano),
		e.Description,
		e.Reference,
		string(e.Source),
		e.CreatedBy,
...
func (s *Store) loadPostings(ctx context.Context, entryID journal.EntryID) ([]journal.Posting, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, account_id, amount, currency, memo FROM postings WHERE journal_entry_id = ?`,
		string(entryID),
	)
	if err != nil {
...
func scanEntry(s entryScanner) (journal.JournalEntry, error) {
	...
	var id, hhID, postedAtStr, desc, ref, src, createdBy string
	var reversalOf sql.NullString

	err := s.Scan(&id, &hhID, &postedAtStr, &desc, &ref, &src, &createdBy, &reversalOf)
	if err != nil {
		return journal.JournalEntry{}, err
	}

	postedAt, err := time.Parse(time.RFC3339Nano, postedAtStr)
	if err != nil {
		return journal.JournalEntry{}, fmt.Errorf("parse posted_at %q: %w", postedAtStr, err)
	}
...
func buildJournalQuery(q storeapi.JournalQuery) (string, []any) {
	...
	if q.HouseholdID != "" {
		where = append(where, "je.household_id = ?")
		args = append(args, string(q.HouseholdID))
	}
	if !q.After.IsZero() {
		where = append(where, "je.posted_at >= ?")
	...
	if q.DescriptionLike != "" {
		where = append(where, "je.description LIKE ? COLLATE NOCASE")
		args = append(args, "%"+q.DescriptionLike+"%")
	}
	if q.AccountID != "" {
		where = append(where, "EXISTS (SELECT 1 FROM postings p WHERE p.journal_entry_id = je.id AND p.account_id = ?)")
