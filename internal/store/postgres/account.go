...
func (s *Store) CreateAccount(ctx context.Context, a account.Account) error {
	...
	if err := SetHouseholdContext(ctx, tx, a.HouseholdID); err != nil {
		return err
	}
	...
	_, err = tx.ExecContext(ctx,
		...
		a.Subtype, string(a.Currency), parentID, a.IsPlaceholder,
	)
	if err != nil {
...
func (s *Store) ListAccounts(ctx context.Context, householdID account.HouseholdID) ([]account.Account, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, household_id, name, type, subtype, currency, parent_id, is_placeholder
		 FROM accounts WHERE household_id = $1 ORDER BY name`,
		string(householdID),
	)
	if err != nil {
...
func scanAccount(s pgScanner) (account.Account, error) {
	var a account.Account
	var id, hhID, name, typ, subtype, cur string
	var parentID sql.NullString

	err := s.Scan(&id, &hhID, &name, &typ, &subtype, &cur, &parentID, &a.IsPlaceholder)
	if err != nil {
